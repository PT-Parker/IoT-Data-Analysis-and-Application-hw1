...
**目標:**
- 讓使用者可以動態調整參數 (斜率 `a`、雜訊 `noise`、資料點數量 `n`)。
- 即時視覺化資料點和迴歸線的變化。
...
noise = st.sidebar.slider("雜訊 (noise)", 0.0, 2.0, 0.5, 0.1)
n_points = st.sidebar.slider("資料點數量 (n)", 50, 500, 100, 10)
b = 10  # 截距 b 固定為 10

# --- 資料生成 ---
@st.cache_data
def generate_data(a, noise, n_points, b):
    """
    根據使用者輸入的參數生成資料
    """
    x = np.random.rand(n_points) * 10  # x 在 0 到 10 之間
    error = np.random.randn(n_points) * noise
    y = a * x + b + error
    return pd.DataFrame({"x": x, "y": y})

data = generate_data(a, noise, n_points, b)
import streamlit as st
import pandas as pd
from sklearn.linear_model import LinearRegression
import altair as alt
...
def render_crisp_dm_prompt():
    st.header("CRISP-DM 流程說明")
    st.markdown("""
    本應用示範如何依照 CRISP-DM 的步驟執行一個簡單的線性迴歸實驗：

    1) 商業理解 (Business Understanding): 說明問題與目的
    2) 資料理解 (Data Understanding): 生成並觀察資料
    3) 資料準備 (Data Preparation): 檢查資料與前處理
    4) 模型建立 (Modeling): 訓練模型
    5) 模型評估 (Evaluation): 評估模型並解釋指標
    6) 部署 (Deployment): 展示與分享結果

    此頁面同時會在側邊欄讓使用者調整生成資料的參數（包含 a, b, noise, n, seed），並在流程中說明每一步的 prompt 與執行過程。
    """)


render_crisp_dm_prompt()

st.sidebar.header("參數設定 (Data Generation)")
a = st.sidebar.slider("斜率 a", -5.0, 5.0, 1.0, 0.1)
...
st.sidebar.markdown("---")
st.sidebar.markdown("提示 (Prompt) - 請說明你要探索的問題，例如：`我想看 a=2, b=5, noise=0.5 下模型的表現`。應用將展示完整流程與結果。")


# 2. Data understanding & generation
@st.cache_data
...
st.markdown("說明：我們使用合成資料 y = a*x + b + noise 以便清晰展示模型行為。下面列出前 5 筆資料：")
